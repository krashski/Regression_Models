...
```
However, the Lotus Europa has a Cook's distance greater than 4 divided by the number of observations, suggesting that this data point may have a high influence on the regression model and should be examined more closely.
```{r echo = FALSE}
...
confint(f4b)[3, ]
```
Finally, the variance inflation factors are all less than 4, suggesting that multicollinearity is not present in the final model.
```{r echo = FALSE}
vif(f4b)
```
